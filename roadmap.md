# RannaKori redesign roadmap

- [ ] Establish premium Bengali design tokens, typography, motion, and responsive shell
- [ ] Redesign home discovery, search, shortcuts, ingredient selection tray, and navigation
- [ ] Redesign pantry and ingredient picker
- [ ] Redesign recipe loading, result, alternatives, missing ingredients, and checklists
- [ ] Redesign full-screen cooking mode and completion flow
- [ ] Redesign favorites, history, authentication, settings, and about pages
- [ ] Update app metadata and remove production badge configuration
- [ ] Verify core flows on mobile and desktop
